...
raceeth <- list(
  n_asian = n_distinct(demo %>% filter(race_collapsed == "Asian")),
  n_blackAA = n_distinct(demo %>% filter(race_collapsed == "Black or African American")),
  n_other = n_distinct(demo %>% filter(race_collapsed == "AI, AN, or other race")),
  n_unknown = n_distinct(demo %>% filter(race_collapsed == "Unknown by patient")),
  n_white = n_distinct(demo %>% filter(race_collapsed == "White")),
  n_oth_unk = n_distinct(demo %>% filter(race_collapsed %in% c("AI, AN, or other race", "Unknown by patient")))
)
...
table1 <- study_pop %>% 
  distinct(StudyID, .keep_all = TRUE) %>%
  tbl_summary(include = c(sex, race_collapsed, hispanic, marital_status, CTCAGE_AT_DIAGNOSIS, CTCDATE_OF_DIAGNOSIS_YYYY, laterality, stage_summary, estrogen_receptor, reporting_source), sort = list(c(sex) ~ "frequency"))
s
...
Figure 1 produced above. It looks like there is a decrease in adherence before the pandemic and another decrease afterward, but these results should be taken with caution as they are not linear and in fact, not statistically significantly different from 0 or each other.
...
labs(title = "White", x = "Year", y = "Proportion Adherent (80%)") +
  scale_y_continuous(limits = c(0.5,1)) + theme_bw()

# Black 
pdc_blackAA <- demo %>%
  filter(race_collapsed == "Black or African American") %>%
  left_join(cma5, by = "StudyID") %>%
  get_cma_results(COVID_start)
blackAA_plot <- pdc_blackAA$reg_plot + 
labs(title = "Black or AA", x = "Year", y = "Proportion Adherent (80%)") +
  scale_y_continuous(limits = c(0.5,1)) + theme_bw()

# Asian 
pdc_asian <- demo %>%
  filter(race_collapsed == "Asian") %>%
  left_join(cma5, by = "StudyID") %>%
  get_cma_results(COVID_start)
asian_plot <- pdc_asian$reg_plot + 
labs(title = "Asian (collapsed)", x = "Year", y = "Proportion Adherent (80%)") +
  scale_y_continuous(limits = c(0.5,1)) + theme_bw()

# TODO eventually remove titles and label using patchwork
plot_by_race <- white_plot / blackAA_plot / asian_plot
plot_by_race
```

They look different, but population sizes differ.
