...
PDC7_grouped <- PDC7_clean %>%
  ...
  summarise(adherent1_count = sum(adherent1 == 1, na.rm = TRUE), 
            ...
            prop_adherent1 = adherent1_count/(adherent1_count + non_adherent1_count + adherent1_NA_count),
          ...
          adherent2_count = sum(adherent2 == 1, na.rm = TRUE), 
            ...
            prop_adherent2 = adherent2_count/(adherent2_count + non_adherent2_count + adherent2_NA_count),
...
its_PDC7_mean1 <- lm(mean_adherent1 ~ window.ID + post_COVID + window.ID*post_COVID, data = PDC7_grouped)
summary(its_PDC7_mean1)
PDC7_grouped$fit_mean1 <- predict(its_PDC7_mean1)

its_PDC7_prop1 <- lm(prop_adherent1 ~ window.ID + post_COVID + window.ID*post_COVID, data = PDC7_grouped)
summary(its_PDC7_prop1)
PDC7_grouped$fit_prop1 <- predict(its_PDC7_prop1)

plot_adherent1 <- ggplot(PDC7_grouped, aes(x = window.start, y = prop_adherent1)) +
  geom_point() +
  geom_line(aes(y = fit_prop1), colour = "blue", linewidth = 1) +
  geom_vline(xintercept = COVID_start, colour = "red") +
  labs(title = "4-week Proportion of Days Covered", x = "Year", y = "Proportion Adherent (80%)") 

its_PDC7_mean2 <- lm(mean_adherent2 ~ window.ID + post_COVID + window.ID*post_COVID, data = PDC7_grouped)
summary(its_PDC7_mean2)
PDC7_grouped$fit_mean2 <- predict(its_PDC7_mean2)

its_PDC7_prop2 <- lm(prop_adherent1 ~ window.ID + post_COVID + window.ID*post_COVID, data = PDC7_grouped)
summary(its_PDC7_prop2)
PDC7_grouped$fit_prop2 <- predict(its_PDC7_prop2)

plot_adherent2 <- ggplot(PDC7_grouped, aes(x = window.start, y = prop_adherent2)) +
  geom_point() +
  geom_line(aes(y = fit_prop2), colour = "blue", linewidth = 1) +
  geom_vline(xintercept = COVID_start, colour = "red") +
  labs(title = "4-week Proportion of Days Covered", x = "Year", y = "Proportion Adherent (80%)") 


plot_adherent1 / plot_adherent2

```

Want to create a function to pipe through some of these since I eventually want to look at these measures among subsets of population. Workflow would be subset -\> pass through function -\> get results
...
# maybe doesn't need to be part of function 
IDs <- studydata %>%
  filter(dispense_dt < FU_end) %>%
  group_by(StudyID) %>%
  slice_max(order_by = dispense_dt, n = 1) %>%
...
# feed into function: 
# FU_start, FU_end, dataset with IDs, COVID_start
...
grouped_data <- dataset %>%
  group_by(window.start) %>%
  summarise(adherent_count = sum(adherent == 1, na.rm = TRUE), 
          ...
          mean_adherent = mean(adherent, na.rm = TRUE),
            prop_NA = adherent_NA_count/(adherent_NA_count + adherent_count + adherent_NA_count)) %>%
  mutate(window.ID = row_number(),
         post_COVID = if_else(window.start >= COVID_start, 1, 0))
 
its_regression <- lm(prop_adherent ~ window.ID + post_COVID + window.ID*post_COVID, data = dataset)
summary(its_regression)

dataset$fit <- predict(its_regression)
...
its_PDC7_22 <- lm(prop_adherent2 ~ window.ID + post_COVID + window.ID*post_COVID, data = PDC7_grouped)
summary(its_PDC7_22)

PDC7_grouped$fit22 <- predict(its_PDC7_22)

plot_adherent2 <- ggplot(PDC7_grouped, aes(x = window.start, y = prop_adherent2)) +
  geom_point() +
  geom_line(aes(y = fit22), colour = "blue", linewidth = 1) +
  geom_vline(xintercept = COVID_start, colour = "red") +
  labs(title = "4-week Proportion of Days Covered*", x = "Year", y = "Proportion Adherent (80%)") 
```
