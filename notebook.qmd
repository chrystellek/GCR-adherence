...
# checking week creation because last one was returning as NA
# summary(cma4_weekly$week)
...
cma4_weekly$fit2 <- predict(its_reg_week, type = "response")

ggplot(cma4_weekly, aes(x = week, y = prop_adherent)) + geom_line() +
  geom_vline(xintercept = pandemy_start_week, colour = "red") + 
  geom_smooth(method = "lm", formula = y ~ x + post_COVID_week + time_passed_weeks, se = FALSE, col = "blue") +  
  labs(x = "Time", y = "Proportion >80%") +  
  theme_classic()  
#  scale_x_continuous(expand = c(0, 0), limits = c(0, 1700)) +
 # scale_y_continuous(expand = c(0, 0), limits = c(0.5, 1)) +
...
results_plot <- ggplot(cma4_weekly, aes(x = week, y = prop_adherent)) +
  geom_point() +  
...
Feb 9: TODO is have figure \# assigned to each chunk, figure out Quarto lol
