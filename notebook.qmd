...
Comparing 3 month window adherence 
```{r}
#| echo: true
#| warning: false
#| label: CMA-12weeks

slide_cma4_3mos <- CMA_sliding_window(CMA.to.apply="CMA4", 
                                 data = studydata,
                                 ID.colname = "StudyID",
                                 event.date.colname = "dispense_dt",
                                 event.duration.colname = "days_supply",
...
cma4_3mos <- as_tibble(slide_cma4_3mos$CMA) %>%
    filter(window.start <= as.Date("2022-07-01", format = "%Y-%m-%d"))
...
its_3mos <- lm(prop_adherent ~ week + post_COVID_week + week*post_COVID_week, data = cma4_3weekly)
summary(its_3mos)

cma4_3weekly$fit <- predict(its_3mos)

ggplot(cma4_3weekly, aes(x = week, y = prop_adherent)) +
  geom_point() +  
  geom_line(aes(y = fit), colour = "blue", linewidth = 1) +
  geom_vline(xintercept = pandemic_start_week, colour = "red") +
 # scale_x_continuous(expand = c(0, 0)) +
#  scale_y_continuous(expand = c(0, 0)) +
  labs(title = "12 Week Adherence", x = "Week", y = "Proportion Adherent (80%)") +  
  theme_classic() 
```
