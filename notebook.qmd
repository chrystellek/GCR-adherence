...
# saving this so I don't have to run each time 
saveRDS(cma4, file = "CMA4_4weeks.rds")
```
...
pandemic_start <- difftime(COVID_start, FU_start, units = "days")

cma4 <- cma4 %>%
  mutate(adherent = case_when(
    CMA >= 0.80 ~ 1,
    CMA < 0.80 ~ 0), 
    time_days = as.numeric(difftime(window.start, FU_start, units = "days")),
         post_COVID = if_else(window.start > COVID_start, 1, 0),
    time_passed_days = if_else(
      post_COVID == 1, as.numeric(difftime(window.start, COVID_start, units = "days")), 
      0)) 

its_reg_daily <- lm(adherent ~ time_days + post_COVID + time_passed_days, data = cma4)
summary(its_reg_daily)

ggplot(cma4, aes(x = time_days, y = adherent)) + geom_point() +
#  geom_line(aes(y = fit), colour = "blue") + 
  geom_vline(xintercept = pandemic_start, colour = "red") + 
...
# want to create 3 new duplicate rows for each windowID
# update window.start 
cma4_test <- cma4 %>%
  group_by(StudyID) %>%
...
#| label: CMA-sandbox
exam <- studydata %>%
   filter(StudyID %in% c(3, 6, 9, 12)) %>%
   select(StudyID, dispense_dt, days_supply, drug_group, daily_dose, AETdrug) %>%
   arrange(StudyID, dispense_dt) %>%
  mutate(dispense_end = dispense_dt + days_supply,
         FU_start = as.Date("2018-01-01", format = "%Y-%m-%d"))

FU_start <- as.Date("2018-01-01", format = "%Y-%m-%d") 

test_cma4 <- CMA_sliding_window(CMA.to.apply="CMA4", 
                                 data = exam,
                                 ID.colname = "StudyID",
                                 event.date.colname = "dispense_dt",
                                 event.duration.colname = "days_supply",
                                 ...
                                 parallel.threads = 1)
test_cma4$CMA
plot(test_cma4, patients.to.plot = c(3, 12))

cma4_test3 <- as_tibble(test_cma4$CMA) 
cma9_test1 <- as_tibble(test1_cma9$CMA)

test_cma <- CMA_sliding_window(CMA.to.apply="CMA8", 
                                 data = exam,
                                 ID.colname = "StudyID",
                                 event.date.colname = "dispense_dt",
                                 event.duration.colname = "days_supply",
                                 ...
                                 consider.dosage.change = FALSE,
                                 followup.window.start = 0,
                                 followup.window.duration = 365*4,
                                 followup.window.duration.unit = "days",
                                 observation.window.start = 0,
                                 observation.window.duration = 365*4,
                                 ...
                                 parallel.threads = 1)
test_cma$CMA
inner_info <- as_tibble(getInnerEventInfo(test_cma))
```
...
whatever <- studydata %>%
   filter(StudyID %in% c(3, 6, 9, 12, 1330)) %>%
   select(StudyID, dispense_dt, days_supply, drug_group, daily_dose, AETdrug) 
