...
April 5: A note about this function/calculating NAs. Not everyone has a line for each window start, so sum of NA doesn't include those who are outside of OW.
...
cma7_short_results <- bind_rows(result_list)
saveRDS(cma7_short_results, "./data/cma7-short.rds")

COVID_start <- as.Date("2020-03-11", format = "%Y-%m-%d")
cma7_short <- get_cma_results(cma7_short_results, COVID_start)

summary(cma7_short$data_summary)
cma7_short$reg_plot
...
summary(its7)
tbl_regression(summary(its7), estimate_fun = ~ style_number(.x, digits = 4))
...
cma7_long_results <- readRDS("./data/cma7.rds")
cma7_long <- get_cma_results(cma7_long_results, COVID_start)
cma7_plot1<- cma7_long$reg_plot + geom_line() +
  labs(title = "4-week PDC (CMA7)", x = "Year", y = "Proportion Adherent (80%)") +
  theme_bw() + scale_y_continuous(limits = c(0.1,1)) 

# OW using end of supply
cma7_short$reg_plot
cma7_plot2 <- cma7_short$reg_plot + geom_line() +
  labs(title = "4-week PDC (CMA7; simpler OW)", x = "Year", y = "Proportion Adherent (80%)") +
  theme_bw() + scale_y_continuous(limits = c(0.1,1)) 

cma7_plot1
cma7_long$reg_summary

### CMA4
result_list <- c()
for (i in unique(study_run$StudyID)){
  study_data <- study_run[study_run$StudyID == i, ]
  result <- run_CMA(study_data, "CMA4")
  result_list[[i]] <- result
}
cma4_results <- bind_rows(result_list)
saveRDS(cma4_results, "./data/cma4.rds")


```

I have more faith in the plots with more complex OW.
...
demo <- study_run_OW %>%
  group_by(StudyID) %>%
  slice(n()) %>%
  select(StudyID, CTCDATE_OF_DIAGNOSIS_YYYY, ERstatus, stage_summary, drug_group, sex:reporting_source, horm_dt) 

early_years <- demo %>%
  filter(CTCDATE_OF_DIAGNOSIS_YYYY %in% c(2015, 2016, 2017)) %>%
  left_join(cma7_long_results, by = "StudyID") %>%
  get_cma_results(COVID_start)

dx2017 <- early_years$reg_plot + 
labs(title = "Year dx 2015-2017", x = "Year", y = "Proportion Adherent (80%)") +
  scale_y_continuous(limits = c(0.1,1)) + geom_line() + theme_bw()

later_years <- demo %>%
  filter(CTCDATE_OF_DIAGNOSIS_YYYY %in% c(2018, 2019)) %>%
  left_join(cma7_long_results, by = "StudyID") %>%
  get_cma_results(COVID_start)

dx2018 <- later_years$reg_plot + 
labs(title = "Year dx 2018-2019", x = "Year", y = "Proportion Adherent (80%)") +
  scale_y_continuous(limits = c(0.1,1)) + geom_line() + theme_bw()

cma7_plot1 / dx2017 / dx2018

# just looking at number adherent
cma7_data <- cma7_long$data_summary
ggplot(cma7_data, aes(x = window.start, y = adherent_count)) +
...
pdc_white$reg_plot + 
labs(title = "NH White PDC", x = "Year", y = "Proportion Adherent (80%)") +
  scale_y_continuous(limits = c(0.1,1)) 
...
pdc_white_data$race_group <- "NH White"
...
pdc_black_data$race_group <- "NH Black"
...
pdc_API_data$race_group <- "NH API"
...
pdc_hisp_data$race_group <- "Hispanic"

pdc_merged_race <- bind_rows(pdc_white_data, pdc_black_data, pdc_API_data, pdc_hisp_data)
