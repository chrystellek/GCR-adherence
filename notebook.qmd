...
study_ids <- study_pop %>% 
  group_by(StudyID) %>%
  slice_head()
table(study_ids$race_collapsed, study_ids$PATIENTNHIA, useNA = "always")
```

Convo with Kevin (3/7)

1.  Tumor record number (in data file) and changed to first filter by ER+ then min tumor number based on KW email response: SeqNum will be added and then I will update and filter by SeqNum
2.  AI or tamoxifen before diagnosis date, created csv below: the handful we looked at are second breast cancers. Will filter by SeqNum.

```{r}
#| include: false
#| label: closer-look
rx_before_dx_all <- pharmstudydata %>%
  filter(dispense_dt <= dt_diagnosis) 
write.csv(rx_before_dx_all, "./data/rx_before_dx.csv")
n_distinct(rx_before_dx)

unique_study_pop <- study_pop %>%
  distinct(StudyID, .keep_all = TRUE)
```

3.  Small numbers: supress cells \<5 and rates for \<15
4.  Urban/rural variables: will be added and uploaded. Census and county level.
5.  Race and ethnicity: NH-White, NH-Black, Hisp/Lat. can see what other races Hisp/Lat overlap.
...
# TODO summarize adherence %s and missing %s
```
...
Some people have their first dispense and/or first post-pandemic dispense way after March 2020. Should they still be included?
