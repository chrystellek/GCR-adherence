...
Chrystelle Kiang Date rendered: 2024-02-26
...
# TODO decide if we should keep people who have a dispense only after covid
cma5 <- read_rds("./data/pdc-cma5.rds")
COVID_start <- as.Date("2020-03-11", format = "%Y-%m-%d")

# WARNING: this IDlist is NOT the same as previous IDlist
covid_IDs <- study_pop %>%
  group_by(StudyID) %>%
  ...
  select(StudyID, post_dispense, days_supply, post_dispense_end)
...
cma52 <- cma5 %>%
  ...
  mutate(post_dispense_end = if_else(is.na(post_dispense_end), COVID_start, post_dispense_end)) %>%
...
cma52 %>% group_by(StudyID) %>% slice(n()) %>%
ggplot(aes(x = first_dispense)) +
  geom_histogram()

# TODO sort out the window IDs to be more aligned
# this version has 245 window IDs vs. previously had 52
# may need to do some rounding, maybe seeking closest window.start? 

cma5_new <- cma52 %>%
    mutate(adherent = case_when(
    CMA >= 0.80 ~ 1,
    CMA < 0.80 ~ 0),
    # create new windowID according to first fill due after COVID
    new_window_ID = as.integer(difftime(window.start, post_dispense_end, units = "weeks"))) %>% 
  ...
  reframe(adherent_count = sum(adherent == 1, na.rm = TRUE), 
            ...
            prop_NA = adherent_NA_count/(adherent_NA_count + adherent_count + adherent_NA_count),
          inverse_prop_NA = 1 - prop_NA,
          post_COVID = if_else(window.start >= post_dispense_end, 1, 0))

its_cma5 <- lm(prop_adherent ~ new_window_ID + post_COVID + new_window_ID*post_COVID, data = cma5_new)
summary(its_cma5)

# have some times that are missing prop_adherent
cma5_new2 <- cma5_new %>% na.omit(prop_adherent) %>%
  mutate(total_count = adherent_count + non_adherent_count + adherent_NA_count)
  
cma5_new2$fit <- predict(its_cma5)

ggplot(cma5_new2, aes(x = new_window_ID, y = prop_adherent)) +
  geom_point() +
  geom_line(aes(y = fit), colour = "blue", linewidth = 1) +
  geom_vline(xintercept = 0, colour = "red") +
  labs(title = "4-week PDC (CMA7)", x = "Time", y = "Proportion Adherent (80%)") +
  scale_y_continuous(limits = c(0,1), expand = c(0, 0)) 
```
